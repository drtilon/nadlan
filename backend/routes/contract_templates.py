...
def get_templates():
    ...
    try:
        ...
        return jsonify({"message": "Error fetching contract templates", "error": str(e)}), 500
...
def get_template(template_id):
    ...
    try:
        ...
        return jsonify({"message": "Error fetching contract template", "error": str(e)}), 500
...
def add_template():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"message": "Error creating contract template", "error": str(e)}), 500
...
def upload_template():
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"message": "Error uploading contract template", "error": str(e)}), 500
...
def update_template(template_id):
    ...
    try:
        ...
        if name and name != template.name:
            ...
            if existing_template:
                return jsonify({"message": "Another template with this name already exists"}), 400
            template.name = name
        ...
        if is_default is not None and is_default and not template.is_default:
            ...
            ContractTemplate.query.filter(ContractTemplate.id != template_id).update({"is_default": False})
        ...
        db.session.rollback()
        return jsonify({"message": "Error updating contract template", "error": str(e)}), 500
...
def set_default_template(template_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"message": "Error setting default template", "error": str(e)}), 500
...
def delete_template(template_id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"message": "Error deleting contract template", "error": str(e)}), 500
...
def create_contract_with_template():
    ...
    try:
        ...
        buffer = BytesIO()
        update_rental_agreement(template.file_path, buffer, db_data)  # Use template.file_path
        buffer.seek(0)
