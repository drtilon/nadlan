...
def add_individual_payment():
    """Add an individual payment for a specific tenant - FINAL CORRECTED VERSION"""
    try:
        ...
        if not all([apartment_id, amount, payment_date_str]):
            return jsonify(
                {
                    "message": "Missing required fields: apartment_id, amount, payment_date"
                }
            ), 400
        ...
        # FIXED: Convert month name to integer for database storage
        if isinstance(month, str):
            ...
            try:
                ...
                month_int = payment_date.month
        else:
            month_int = month or payment_date.month

        # FIXED: Create tenant data structure for tenant_payments field (not tenants)
        tenant_data = [
            {
                "id": tenant.id,
                "name": tenant.name,
        ...
        # FIXED: Create new payment record using actual Payment model fields
        new_payment = Payment(
            apartment_id=apartment_id,
            month=month_int,  # Use integer month (1-12)
            ...
            electricity=0.0,
            other=0.0,
            tenant_payments=json.dumps(
                tenant_data
            ),  # FIXED: Use tenant_payments not tenants
        ...
        return jsonify(
            {
                ...
                or apartment.full_address
                or f"Apartment {apartment.id}",
                "amount": amount,
                "payment_date": payment_date.isoformat(),
                "payment_type": payment_type,
            }
    ...
    try:
        ...
        if "payment_date" in data:
            try:
                ...
                payment_date_value = datetime.fromisoformat(
                ...
                try:
                    payment_date_value = datetime.strptime(
    ...
    try:
        ...
        for month_name, payment_info in payments_data.items():
            ...
            if existing_payment:
                ...
                if (
                    hasattr(existing_payment, "contract_period_id")
                    and contract_period_id
                ):
                ...
                if (
                    hasattr(new_payment, "payment_date")
                    and status == "paid"
                    and payment_date
                ):
                    new_payment.payment_date = payment_date
                elif (
                    hasattr(new_payment, "paymentDate")
                    and status == "paid"
                    and payment_date
                ):
                    new_payment.paymentDate = payment_date
...
def get_batch_payments_data(apartment_id):
    """
    Get batch payments data - FIXED for actual Payment model fields
    ...
    try:
        ...
        payments = (
            Payment.query.filter_by(apartment_id=apartment_id, year=year_filter)
            .order_by(
                case((Payment.payment_date.is_(None), 1), else_=0).asc(),
                Payment.payment_date.desc(),
            )
            .all()
        )
        ...
        history = []
        for payment in payments:
            # FIXED: Parse tenant_payments data (not tenants) and handle both fields
            tenant_names = []
            tenant_data = []

            # Check tenant_payments field first (correct field)
            if hasattr(payment, "tenant_payments") and payment.tenant_payments:
                try:
                    tenants_list = json.loads(payment.tenant_payments)
                    for tenant_info in tenants_list:
                        if tenant_info.get("name"):
                            tenant_names.append(tenant_info["name"])
                        tenant_data.append(tenant_info)
                except json.JSONDecodeError:
                    tenant_names = ["Unknown"]
            # Fallback to legacy tenants field if it exists
            elif hasattr(payment, "tenants") and payment.tenants:
                try:
                    tenants_list = json.loads(payment.tenants)
                    for tenant_info in tenants_list:
                        if tenant_info.get("name"):
                            tenant_names.append(tenant_info["name"])
                        tenant_data.append(tenant_info)
            ...
            # FIXED: Ensure tenant_names is not empty
            if not tenant_names:
                tenant_names = ["N/A"]

            # Calculate total amounts
            total_paid = (
                sum(float(tenant.get("amountPaid", 0)) for tenant in tenant_data)
                if tenant_data
                else 0
            )
            total_due = (
                sum(float(tenant.get("amountDue", 0)) for tenant in tenant_data)
                if tenant_data
                else 0
            )

            # FIXED: Build proper history entry
            entry = {
                "id": payment.id,
                "month": payment.month,
                "year": payment.year,
                ...
                "tenant_name": ", ".join(
                    tenant_names
                ),  # FIXED: Join multiple tenant names
                "tenant_names": tenant_names,  # FIXED: Array of tenant names
                "notes": getattr(payment, "notes", ""),
                "isIndividual": True,  # Individual payments from tenant page
                "tenants": tenant_data,  # Full tenant data
            }
            history.append(entry)
